...
$totalQuery = "SELECT COUNT(*) as total FROM tickets WHERE user_id = ?";
...
if (!empty($search_id)) {
    ...
    $stmt->bind_param("siii", $userid, $search_id, $limit, $offset);
} else {
    // Caso contrário, busca todos os tickets do usuário
    $ticketsQuery = "SELECT id, description, open_date, category 
                     FROM tickets 
                     WHERE user_id = ? 
                     ORDER BY id DESC 
